...
const SUBS = {
  normal: {
    ...
    uriLabel: 'Обычный',
    emoji: '🌍',
    // The #fragment sets the subscription name shown in v2Box, Happ etc.
    cdnUrl:
...
const serviceLabel = 'BestyNetPortal';
Object.values(SUBS).forEach((sub) => {
  const fragment = `${serviceLabel}: ${sub.uriLabel}`;
  sub.cdnUrl += encodeURIComponent(fragment);
});
...
function buildDeepLink(cdnUrl: string, clientId: string): string {
  ...
  switch (clientId) {
    case 'v2box':
      return `v2box://install-sub?url=${encoded}`;
    case 'v2ray':
      return `v2rayng://install-config?url=${encoded}`;
    case 'happ':
      return `happ://add/${encoded}`;
    case 'streisand':
      return `streisand://import/${b64}`;
    case 'nekobox':
      return `clash://install-config?url=${encoded}`;
    case 'shadowrocket':
      return `sub://${b64}`;
    default:
      return cdnUrl;
...
// The external browser CAN open custom schemes (v2box://, happ://) unlike Mini App webview
function getRedirectPageUrl(deeplink: string): string {
  const base = window.location.href.split('#')[0];
  return `${base}#/open/${encodeURIComponent(deeplink)}`;
}

// Client button: deeplinks from Telegram Mini App context
function ClientBtn({
  clientId,
  subType,
}: {
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    // The redirect page runs outside the Mini App context (in Safari/Chrome)
    // and can freely navigate to custom schemes like v2box://, happ://, etc.
    const redirectPage = getRedirectPageUrl(deeplink);
    window.Telegram!.WebApp!.openLink(redirectPage, {
...
export function EasyMode({ onDone, resetSignal }: Props) {
  ...
  async function runCheck() {
    ...
    const ok = await probe('https://www.google.com/favicon.ico').catch(
      () => false,
    );
  ...
  if (!showResult) {
    return (
      <div className='easy-idle'>
        ...
        <p className='easy-hint-text'>
          Нажмите — подберём нужный вариант за пару секунд
        </p>
        ...
        <div className='easy-manual-wrap'>
          ...
          <div className='easy-manual-row'>
            <button
              className='easy-manual-pill pill-normal'
              onClick={() => selectManual('normal')}
            >
              🌍 Обычный список
            </button>
            <button
              className='easy-manual-pill pill-white'
              onClick={() => selectManual('whitelist')}
            >
              🗺️ Белый список
            </button>
            <button
              className='easy-manual-pill pill-outline'
              onClick={() => selectManual('outline')}
            >
              🔑 Outline
            </button>
          </div>
  ...
  return (
    <div className={`easy-result ${animating ? 'fade-out' : 'fade-in'}`}>
      <div
        className='easy-result-card'
        style={{ '--rc': sub.color } as React.CSSProperties}
      >
        {/* Header */}
      ...
      <div className='easy-fallback-block'>
        ...
        <p className='easy-fallback-desc'>
          Попробуйте другой список — иногда один вариант работает лучше в
          зависимости от вашей сети.
        </p>
          ...
          {(['normal', 'whitelist', 'outline'] as SubType[]).map((type) => (
            <button
              ...
              style={
                activeSub === type
                  ? {
                      borderColor: SUBS[type].color,
                      color: SUBS[type].color,
                      background: `${SUBS[type].color}18`,
                    }
                  : {}
              }
      ...
      <p className='easy-amnezia-note'>
        🔒 Ключи AmneziaWG в этом репозитории не публикуются — для Amnezia
        используйте встроенный поиск серверов внутри приложения.
      </p>
    </div>
  );
}
    ...
    return (
      <div className='outline-load-wrap'>
        <p className='outline-desc'>
          SS-ключи совместимы с <strong>Outline</strong>, Shadowsocks и другими
          клиентами с поддержкой протокола SS.
        </p>
      ...
      <div className='outline-list'>
        {filtered.map((k, i) => (
          <div key={i} className='outline-key-row'>
            <code className='outline-key-text'>
              {k.slice(0, 72)}
              {k.length > 72 ? '…' : ''}
            </code>
            <button
              className={`outline-copy ${copied[i] ? 'ok' : ''}`}
              onClick={() => copyKey(k, i)}
            >
              {copied[i] ? '✓' : '⎘'}
            </button>
